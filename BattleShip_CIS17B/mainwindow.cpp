...
void MainWindow::handleButton(){
    ...
    if(!hold){
        ...
        if (tempShip.getBadPlacement()){
            if(debugging){ qDebug() << "Placement of the " << cf.index2ShipName(drops) << " was off the grid. Try again!"; }
        ...
        if (coordinates.size() > 0){
            for (int w = 0; w < signed(tempCoordinates.size()); w++){
                ...
                if (it != coordinates.end()){	//A coordinate of the temporary Ship has matched a coordinate from another ship.
                    if(debugging){ qDebug() << "Placement of the " + cf.index2ShipName(drops) + " was on top of another ship. Try again!"; }
                    return;
                }
            }
        }
...
void MainWindow::createPlayerGrid(){
    ...
    //Set spacing to zero so that there is a small space between cells
    ui->playerGrid->setSpacing(1);
    ui->playerGrid->setVerticalSpacing(5);
    // Vertical spacers
        ...
        //if(playerTurn){
            ...
            if(attackOK){
                ...
                if (result == "0"){	//Miss
                    ...
                    if(debugging){ qDebug() << attack << " was a hit! " << shipName << " was sunk!"; }
                    enemyCells[count]->setStyleSheet("background-color: red");
                    if(debugging){ qDebug() << "Enemy ships left: " + QString::number(enemy.getShipsAlive()); }
                }
                ...
                if(debugging){ qDebug() << attack + " has already been used. Try a different coordinate"; }
            }
            ...
            while(!attackOK){
                ...
                if(attackOK){
                    ...
                    //result = temp.substr(0, 1);
                    result = temp.mid(0, 1); //QT way
                    if(debugging){ qDebug() << "Cell: " + attack + "   Enemy Result" + result + "  Index: " + QString::number(index); }
                    //Tell the player what the result of his/her attack was
                    if (result == "0"){	//Miss
                        if(debugging){ qDebug() << attack << " was a miss."; }
                        ...
                        if(debugging){ qDebug() << "Player ships left: " + QString::number(player.getShipsAlive()); }
                    }
